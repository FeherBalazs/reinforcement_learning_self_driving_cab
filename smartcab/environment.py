...
class Environment(object):
    ...
    def __init__(self):
        self.done = False
        self.dest_reached_counter = 0 # added by me
        self.trial_failed = [] # added by me
        self.t = 0
    ...
    def step(self):
        ...
        if self.primary_agent is not None:
            if self.enforce_deadline and self.agent_states[self.primary_agent]['deadline'] <= 0:
                self.done = True
                self.trial_failed.append(self.simulator.get_trial_failed()) # added by me
    ...
    def get_dest_reached(self):
        return self.dest_reached_counter
    
    def get_trial_failed(self):
        return self.trial_failed
        ...
        if agent is self.primary_agent:
            if state['location'] == state['destination']:
                if state['deadline'] >= 0:
                    reward += 10  # bonus
                self.done = True
                self.dest_reached_counter += 1 # added by me
                print "Environment.act(): Primary agent has reached destination {} times!".format(self.dest_reached_counter)  # [debug]
